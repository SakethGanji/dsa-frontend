            ...
            {isListView ? (
                <>
                    <div className="flex flex-col w-64 md:w-72 flex-shrink-0">
                        <CardHeader className="p-3 pb-0">
                            <div className="flex justify-between items-start gap-2">
                                <motion.div layoutId={`title-${dataset.id}-${id}`} className="flex-grow">
                                    <CardTitle className="text-sm line-clamp-1">
                                        {dataset.name}
                                    </CardTitle>
                                </motion.div>
                                <motion.div layoutId={`badge-${dataset.id}-${id}`}>
                        ...
                        <CardContent className="px-3 pt-0 pb-3">
                            <motion.div layoutId={`description-${dataset.id}-${id}`}>
                                <CardDescription
                            ...
                            <motion.div layoutId={`info-${dataset.id}-${id}`} className="mt-2">
                    ...
                    <div className="flex-grow flex flex-col justify-between border-l">
                        ...
                        <CardFooter className="p-3 flex justify-between items-center gap-2 border-t">
                            ...
                            <motion.div layoutId={`actions-${dataset.id}-${id}`} className="flex gap-2">
                                {/* Save */}
                ...
                <>
                    ...
                    <CardContent className="px-4 pt-2 pb-2 flex-grow space-y-2">
                        <motion.div layoutId={`info-${dataset.id}-${id}`} className="flex flex-col gap-2">
                            ...
                            <div className="flex justify-between items-center flex-wrap gap-2 text-sm text-muted-foreground">
                                <motion.div className="flex flex-wrap gap-2">
                                    {dataset.tags.slice(0, isExpanded ? dataset.tags.length : 3).map((tag, i) => (
                                        <motion.div
                                            key={tag}
                                            initial={{ opacity: 0, y: 5 }}
                                            animate={{
                                    ...
                                    {!isExpanded && dataset.tags.length > 3 && (
                                        <motion.div
                                            initial={{ opacity: 0, y: 5 }}
                                            animate={{
                                                opacity: 1,
    ...
    return (
        <>
            ...
            {datasets.map((dataset) => (
                ...
                >
                    {renderDatasetCard(dataset, false, isList)}
                </motion.div>
            ))}
        </>
    )
